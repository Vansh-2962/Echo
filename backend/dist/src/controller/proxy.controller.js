...
export async function createRequest(req, res) {
    try {
        ...
        const bodySize = Buffer.byteLength(typeof apiResponse.body === "string"
            ? apiResponse.body
...
export async function saveRequest(req, res) {
    try {
        ...
        const user = req.user;
        const { method, url, params, headers, bodyType, bodyContent, bodyFormData, authType, authConfig, preRequestScript, postResponseScript, } = data.request;
        ...
        const result = await prisma.$transaction(async (tx) => {
            const request = await tx.requests.create({
                data: {
                    ...
                    params: {
                        create: params
                            .filter((p) => p.key && p.key.trim() !== "" && p.enabled)
                            .map((p) => ({
                            key: p.key,
                            value: p.value,
                    ...
                    headers: {
                        create: headers
                            .filter((h) => h.key && h.key.trim() !== "" && h.enabled)
                            .map((h) => ({
                            key: h.key,
                            value: h.value,
                    ...
                    bodyFields: {
                        create: bodyFormData
                            .filter((bd) => bd.key && bd.key.trim() !== "" && bd.enabled)
                            .map((bd) => ({
                            key: bd.key,
                            value: bd.value,
...
export async function renameRequest(req, res) {
    try {
        const { name } = req.body;
